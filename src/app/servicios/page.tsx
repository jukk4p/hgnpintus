    ...
    <main className="min-h-screen">
      ...
      <section className="py-12 md:py-24 bg-background">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mb-12 md:mb-16">
            <h1 className="text-3xl md:text-7xl font-black uppercase tracking-tighter mb-4 md:mb-6 leading-[0.95] md:leading-[0.9] text-primary">
              NUESTROS SERVICIOS <br />
              <span className="text-accent">PROFESIONALES</span>
            </h1>
            <p className="text-base md:text-xl font-bold text-foreground/80">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {allServices.map((service, index) => {
              ...
              return (
                <div key={index} className="brutalist-card flex flex-col bg-white hover:border-primary transition-all duration-300 group h-full shadow-lg">
                  <div className="relative aspect-square mb-4 md:mb-6 border-2 border-black overflow-hidden shrink-0">
                    ...
                    <div className="absolute top-2 left-2 bg-primary text-white p-2 border-2 border-black z-10 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                      <Icon size={18} className="md:w-5 md:h-5 text-accent" />
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col px-4 pb-6">
                    <div className="mb-3 md:mb-4">
                      ...
                      <h3 className="text-sm md:text-lg font-black uppercase mt-1 leading-tight tracking-tighter text-primary">
                        {service.title}
                      </h3>
                    </div>
                    <p className="text-[11px] md:text-xs font-medium text-foreground/70 leading-relaxed mb-6 line-clamp-4">
                      {service.description}
                    </p>
                    <div className="mt-auto pt-4 border-t-2 border-dashed border-black/10 flex justify-between items-center">
                      <span className="font-black text-[9px] md:text-[10px] uppercase text-foreground/60 tracking-tight">{service.price}</span>
      ...
      <section className="py-16 md:py-24 bg-primary text-white border-y-4 border-black">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-xl md:text-4xl font-black uppercase mb-6 md:mb-8 italic leading-tight text-accent">"Trabajo impecable, rápidos y muy limpios en el mantenimiento de nuestro bloque."</h2>
          <p className="text-sm md:text-base font-bold opacity-70 text-white/80">— Administrador de Fincas, Coria del Río</p>
        </div>
      </section>

      <footer className="py-12 bg-white text-black border-t-4 border-black">
        <div className="container mx-auto px-4 text-center space-y-4">
          <h3 className="text-2xl md:text-3xl font-black uppercase tracking-tighter italic text-primary">HGNPINTURAS</h3>
          <p className="text-xs md:text-sm font-bold opacity-80 text-foreground/80">© {new Date().getFullYear()} Hermanos Gómez Novo. Calidad y Confianza.</p>
          <div className="flex justify-center gap-4 md:gap-6 pt-4 flex-wrap">
            <Link href="/aviso-legal" className="hover:text-primary transition-colors font-black uppercase text-[10px] md:text-xs text-foreground/60">Aviso Legal</Link>
            <Link href="/privacidad" className="hover:text-primary transition-colors font-black uppercase text-[10px] md:text-xs text-foreground/60">Privacidad</Link>
            <Link href="/cookies" className="hover:text-primary transition-colors font-black uppercase text-[10px] md:text-xs text-foreground/60">Cookies</Link>
          </div>
        </div>
      </footer>
